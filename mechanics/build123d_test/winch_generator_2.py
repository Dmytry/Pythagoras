...
def winch_path(phi_to_cable, max_phi, phi_count, decimate, spacing, expand_r, z_offset):
    ...
    for i in range(1, phi_count+2):
        ...
        rfade=min(fade, fade2)

        new_point=(math.cos(phi)*(r+expand_r*rfade), math.sin(phi)*(r+expand_r*rfade), z+z_offset*rfade - spacing*(1-fade2))
...
def ridge_fadeout(r, start_z, max_phi, phi_count, decimate, spacing, expand_r, z_offset):
    z=start_z
    z_per_phi=spacing/(2*math.pi)
    result=[]
    tangents=[]
    ...
    for i in range(1, phi_count+2):
        phi=i*max_phi/phi_count
        z=phi*z_per_phi
        fade=min((max_phi-phi)/(max_phi), 1)
...
integrator_steps_per_rotation=300
steps_per_point=1+integrator_steps_per_rotation//points_per_rotation
...
ridge_pts, ridge_tangents = winch_path(test_cable_fun, max_phi, steps_per_point*points_per_rotation*rotations, decimate, spacing, spacing*0.5, -spacing/2)
...
with BuildPart() as blocks:
    ...
    top_circle_pt=pts[-points_per_rotation-1]
    ...
    faces_down=[Face.make_surface_from_curves(edges_ridge[i], edges_groove[i]) for i in range(0, len(edges_groove)-splines_per_rotation) ]
